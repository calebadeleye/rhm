import { addStationDays, getStationDateInfo, stationLocalMinutesToInstant, type StationDateInfo } from "@/lib/timezone";
 ...
 * station-local calendar date. Non-wrapping slots (end after start) yield
 * at most one occurrence. Overnight-wrapping slots (e.g. 22:00–05:00) are
 * split into a "tail" occurrence (00:00 through the wrap-end, carried over
 * from the previous day's start) and a "head" occurrence (the new start
 * through midnight) — the same convention used by typical radio schedule
 * grids, so a single night block reads as two rows bracketing each day. */
...
export function withStatus(occurrences: RawOccurrence[], now: Date): DaypartOccurrence[] {
  return occurrences.map((occ) => ({
    ...occ,
    status: now.getTime() < occ.start.getTime() ? "upcoming" : now.getTime() > occ.end.getTime() ? "ended" : "live",
  }));
}
...
/** Formats minutes-since-midnight as a 24-hour "HH:mm" label, independent
 * of any timezone conversion — used for the schedule grid's time column,
 * which shows the raw slot boundary rather than a converted instant. */
...
/** Formats minutes-since-midnight as a 12-hour "h:mm AM/PM" label (same
 * station-timezone convention as formatMinutes24), used for the more
 * conversational "On Air Now" / "Next" summary card. */
